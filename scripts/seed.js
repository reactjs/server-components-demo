...
const insertNoteStatement = `INSERT INTO notes(title, body, created_at, updated_at)
  ...
  RETURNING *`
const seedData = [
  ['Meeting Notes', 'This is an example note. It contains **Markdown**!', randomDateBetween(startOfThisYear, now)],
  [
    'Make a thing',
...
async function seed() {
  ...
  await Promise.all(
    oldNotes.filter(filename => filename.endsWith('.md')).map(filename => unlink(path.resolve(NOTES_PATH, filename)))
  )
